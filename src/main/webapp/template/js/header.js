        ...
        if(message.type == 'info'){
            ...
            document.addEventListener('keydown', async function(event) {
                if(event.keyCode == 13){
                    ...
                    await fetch("/MovieWebsite/api/registration/confirm?token=" + codeToken.value, {
                        method : 'POST'
                    }).then((response) => {
                        ...
                        if(message.type.toLowerCase() === 'success'){
                            ...
                            setSignMessageBox(loginSection.querySelector('form'), message.type, message.content);
                        } else {
                            setSignMessageBox(SignupSection.querySelector('form'), message.type, message.content);
                        }
                    }).catch((err) => {
                        setSignMessageBox(SignupSection.querySelector('form'), "danger", err);
                    });
                    stopAnimation();
                }
            });
